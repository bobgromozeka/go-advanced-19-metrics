...
func Test_IsValidSum(t *testing.T) {
	type fields struct {
		key string
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got := IsValidSum(tt.args.sum, tt.args.value, tt.fields.key); got != tt.want {
					t.Errorf("IsValidSum() = %v, want %v", got, tt.want)
				}
			},
		)
...
func Test_Sha256(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got := Sign(tt.fields.key, []byte(tt.args.value)); got != tt.want {
					t.Errorf("Sha256() = %v, want %v", got, tt.want)
				}
			},
		)
